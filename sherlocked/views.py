...
def home(request):
	...
	if request.user.is_authenticated():
		...
		return render_to_response("first.html",{"user":0},context_instance=RequestContext(request))
...
def mystery(request):
	if request.user.is_authenticated():
		...
		ques_len = len(Question.objects.all())
		if user.CurrentQuestionNo != 0:
			...
			if user.CurrentQuestionNo==ques_len+1:
